...
``python run_server.py`` while keeping the main implementation inside
``PyLorex.library.server.simple_tcp``. All command line options supported by the
...
This mirrors the ``python -m PyLorex.library.server.simple_tcp`` entry point
but can be easier to remember when deploying onto a lab machine.
"""

from PyLorex.library.server.simple_tcp import main as _main


def main() -> None:
    """Delegate to :func:`PyLorex.library.server.simple_tcp.main`."""
