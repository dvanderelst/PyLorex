...
``PyLorex.server.simple_tcp``. All command line options supported by the
...
This mirrors the ``python -m PyLorex.server.simple_tcp`` entry point
but can be easier to remember when deploying onto a lab machine.
"""

from PyLorex.server.simple_tcp import main as _main


def main() -> None:
    """Delegate to :func:`PyLorex.server.simple_tcp.main`."""
